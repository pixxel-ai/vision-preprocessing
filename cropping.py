...
class CropAndSave(object):
    ...
    def crop_image(self, img, is_mask=False):  # iterates through list of crops generated from image
        ...
        if self.height_division > 1:
            overlap_rows = (self.size * self.height_division - (np.shape(img))[0]) / (
                        self.height_division - 1)  # overlap between crops along height
        ...
        if self.width_division > 1:
            overlap_column = (self.size * self.width_division - (np.shape(img))[1]) / (
                        self.width_division - 1)  # overlap between crops along width
        ...
        if overlap_rows < 0 or overlap_column < 0:
            raise ArithmeticError(
                'Crop overlaps are negative. Please reconsider the cropping and size parameters specified')
    ...
    def presize(self, img):
        if self.pre_resize == 'spatial':
            self.pre_resize = (
            int(img.shape[1] * self.spatial_scale_factor), int(img.shape[0] * self.spatial_scale_factor))
        if self.pre_resize:
    ...
    def process(self):
        if __name__ == '__main__':
            ...
            img_path_list = [pth for pth in
                             self.IM_FOLDER.iterdir()]  # Iterates over the list of images and passes it to processing functionself.make_dir(self.OUTPUT_FOLDER)
            ...
            results = p.map(self.processing, tqdm(img_path_list))
            results.get()
