...
class ResizeFolder(object):
    ...
    def process(self):
        # distribute_process.map(self.process_one_image, tqdm( list( self.IM_FOLDER.iterdir() ) ))
        # distribute_process.map(self.process_one_mask, tqdm( list( self.MSK_FOLDER.iterdir() ) ))
    ...
    def process_one_item(self, img_path, is_mask=False):
        try:
            ...
            raise Exception('Problem with processing image')
